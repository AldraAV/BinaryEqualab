    ...
    const executeCustomOperation = async () => {
        ...
        try {
            ...
            setResults(prev => [...prev, { 
                id: Date.now(), 
                command: customOperation, 
                result: `\\text{${error.message || 'Error de cálculo. Revisa la operación.'}}`,
                exprRaw: '' 
            }]);
        } finally {
            setLoading(false);
    ...
    const handleExplain = async (command: string, exprRaw: string) => {
        ...
        try {
            ...
            if (response.status === 401) {
                setExplainContent('⚠️ **Requiere Autenticación**\n\nNecesitas iniciar sesión o crear una cuenta gratuita para usar las explicaciones paso a paso con Inteligencia Artificial.');
                setExplainLoading(false);
                return;
            }
            if (response.status === 429) {
                setExplainContent('⚠️ **Límite de Consultas Alcanzado**\n\nHas excedido el límite de consultas de Inteligencia Artificial para tu plan actual.');
                setExplainLoading(false);
                return;
            }
                ...
                <div className="bg-background-light border border-aurora-border rounded-xl p-5 shadow-sm mt-2">
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    if (autocompletado.sugerencias.length > 0) {
                                        e.preventDefault();
                                        const nuevovalor = autocompletado.aplicarSugerencia(0);
                                        setCustomOperation(nuevovalor?.nuevaEntrada || '');
                                        setTimeout(() => {
                                            if (inputRef.current) {
                                                const nuevaPos = nuevovalor?.nuevoCursor || 0;
                            ...
                            <div className="absolute left-0 bottom-full mb-1 bg-background border border-aurora-border rounded-lg shadow-lg overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                                {autocompletado.sugerencias.map((sug, idx) => (
                                    <div
                                        key={sug.name}
                                        onClick={() => {
                                            const nuevovalor = autocompletado.aplicarSugerencia(idx);
                                            setCustomOperation(nuevovalor?.nuevaEntrada || '');
                                            setTimeout(() => {
                                                if (inputRef.current) {
                                                    const nuevaPos = nuevovalor?.nuevoCursor || 0;
                                    ...
                                    >
                                        <div className="flex justify-between items-center gap-4">
                                            <span className="text-primary font-bold">{sug.name}</span>
                                            <span className="text-xs text-aurora-muted italic">{sug.description.es}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-aurora-border/50 flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <button
                                onClick={() => {
                                    setCustomOperation(prev => prev + 'identidad(');
                                    inputRef.current?.focus();
                                }}
                                className="px-3 h-8 rounded bg-white/5 hover:bg-white/20 text-white font-mono text-sm transition-colors border border-aurora-border flex items-center justify-center"
