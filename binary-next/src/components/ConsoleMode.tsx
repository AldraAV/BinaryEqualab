...
const ConsoleMode: React.FC = () => {
  ...
  const obtenerVariablesProtegidas = (expresion: string): Set<string> => {
    ...
    funcionesSimbolicas.forEach(func => {
      ...
      while ((coincidencia = (regex as any)['exec'](expresion)) !== null) {
  ...
  const substituteUserFunctions = (expr: string): string => {
    ...
    for (const [name, fnObj] of Object.entries(userFunctions)) {
      ...
      let match;
      while ((match = (regex as any)['exec'](result)) !== null) {
  ...
  const evaluateWithBackend = async (expr: string): Promise<{ latex: string; rawValue: string; approxResult?: string }> => {
    try {
      ...
      if (processedExpr.startsWith('simplify(')) {
        ...
        const inner = extractFunctionContent(processedExpr, funcName);
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/ai/explain`, {
            method: 'POST',
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={item.plotData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                          ...
                          <Tooltip 
                            ...
                            formatter={(value: any) => [Number(value).toFixed(4), 'y']}
                          />
          ...
          <div className="glass-panel border-t-0 border-r-0 border-l-0 shrink-0 z-20 shadow-[0_-15px_30px_inset_rgba(0,0,0,0.4)]">
            ...
            <div className="px-4 pb-4 pt-1">
              <div className="relative flex items-center">
                <div className={`absolute left-4 z-10 text-xl font-bold font-serif italic ${parseError ? 'text-red-500' : 'text-primary'}`}>ƒ</div>
                
                {/* Ghost Text Overlay */}
                {autocompletado.mostrar && autocompletado.sugerencias.length > 0 && (
                  <div className="absolute inset-0 left-12 right-4 flex items-center pointer-events-none z-0 overflow-hidden whitespace-pre">
                    <span className="text-xl lg:text-2xl font-mono text-transparent">{input}</span>
                    <span className="text-xl lg:text-2xl font-mono text-aurora-muted/40">
                      {autocompletado.sugerencias[autocompletado.indiceSeleccionado].name.substring(autocompletado.palabraActiva.length) + '('}
                    </span>
                  </div>
                )}
                ...
                <input
                  ...
                  className={`relative z-10 w-full bg-transparent border rounded-2xl py-4 pl-12 pr-4 text-xl lg:text-2xl font-mono text-white placeholder:text-aurora-muted focus:ring-2 focus:border-transparent transition-all shadow-inner ${parseError
              ...
              {/* Status Bar for Signature */}
              {autocompletado.mostrar && autocompletado.sugerencias.length > 0 && (
                <div className="mt-2 flex items-center gap-2 text-sm px-2 animate-in fade-in slide-in-from-top-2 duration-200">
                  <Sparkles size={14} className="text-primary" />
                  <span className="font-mono text-primary font-bold">{autocompletado.sugerencias[autocompletado.indiceSeleccionado].syntax}</span>
                  <span className="text-aurora-secondary text-xs truncate"> — {autocompletado.sugerencias[autocompletado.indiceSeleccionado].description.es}</span>
                </div>
              )}
