...
export function usarAutocompletado(entrada: string, posicionCursor: number) {
  ...
  const aplicarSugerencia = (sugerenciaOIndice?: number | FunctionDef): { nuevaEntrada: string; nuevoCursor: number } | null => {
    if (!estado.mostrar || estado.sugerencias.length === 0) return null;
    
    let item: FunctionDef;
    if (typeof sugerenciaOIndice === 'number') {
        item = estado.sugerencias[sugerenciaOIndice];
    } else if (sugerenciaOIndice) {
        item = sugerenciaOIndice;
    } else {
        item = estado.sugerencias[estado.indiceSeleccionado];
    }
