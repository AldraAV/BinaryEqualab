...
Expr num(Real value) { return std::make_shared<Number>(value); }
Expr sym(const std::string& name) { return std::make_shared<Symbol>(name); }
Expr add(Expr a, Expr b) { return std::make_shared<BinaryOp>(a, b, Expression::Type::ADD); }
Expr sub(Expr a, Expr b) { return std::make_shared<BinaryOp>(a, b, Expression::Type::SUBTRACT); }
Expr mul(Expr a, Expr b) { return std::make_shared<BinaryOp>(a, b, Expression::Type::MULTIPLY); }
Expr div(Expr a, Expr b) { return std::make_shared<BinaryOp>(a, b, Expression::Type::DIVIDE); }
Expr pow(Expr base, Expr exp) { return std::make_shared<BinaryOp>(base, exp, Expression::Type::POWER); }
Expr neg(Expr a) { return mul(num(-1.0), a); }
Expr sin(Expr x) { return std::make_shared<Function>("sin", x); }
Expr cos(Expr x) { return std::make_shared<Function>("cos", x); }
Expr tan(Expr x) { return std::make_shared<Function>("tan", x); }
Expr exp(Expr x) { return std::make_shared<Function>("exp", x); }
Expr log(Expr x) { return std::make_shared<Function>("log", x); }
Expr sqrt(Expr x) { return std::make_shared<Function>("sqrt", x); }
Expr abs(Expr x) { return std::make_shared<Function>("abs", x); }
